...
export default async function TtrpgLorePage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
  ...
  const breadcrumbLd = buildBreadcrumbJsonLd([
    ...
    { name: "Lore", url: `${siteUrl}/ttrpg?tab=lore` },
    {
      name: entry.title,
      url: `${siteUrl}/ttrpg/lore/${entry.slug}`,
    },
  ])
      ...
      <article className="max-w-3xl min-w-0">
        <Breadcrumb
          items={[
            ...
            { label: "Lore", href: "/ttrpg?tab=lore" },
            { label: entry.title },
          ]}
        />
        ...
        <header className="mt-4 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
            <Badge variant="secondary">
              {capitalizeCategory(entry.category)}
            </Badge>
            {campaign ? (
              <Link
                href={`/ttrpg?tab=lore&campaign=${campaign.id}`}
